...
namespace GYMHECTORAPI.Controllers
{
    ...
    public class UsuarioController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("ListarAsistenciasDashboard")]
        public async Task<IActionResult> ListarAsistenciasDashboard()
        {
            try
            {
                var principal = User;
                ...
                var idUsuario = Convert.ToInt32(principal.FindFirst(ClaimTypes.NameIdentifier).Value);
                var respuesta = await _iUsuarioBO.ListarAsistenciasDashboard(idUsuario);
                _log.LogInformation("{ListarAsistenciasDashboard} Response: " + JsonSerializer.Serialize(respuesta));
                if (respuesta != null)
                {
                    if (respuesta.codigoRes == HttpStatusCode.OK)
                    {
                ...
                {
                    return StatusCode((int)HttpStatusCode.InternalServerError, new { MessageError = "Error interno en el servicio de listar asistencia dashboard.", MessageUser = "Error. Vuelva a intentarlo." });
                }
            }
            catch (Exception ex)
            {
                _log.LogError("{ListarAsistenciasDashboard} Error: " + ex.ToString());
                return StatusCode((int)HttpStatusCode.InternalServerError, new { MessageError = "Error interno en el servicio de listar asistencia dashboard.." + ex.ToString(), MessageUser = "Error al cargar. Vuelva a intentarlo." });
            }
            finally
            {
                DisposeResources();
